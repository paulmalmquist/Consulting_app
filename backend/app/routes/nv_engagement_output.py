...
from app.schemas.nv_engagement_output import NvContextOut
from app.services import env_context

router = APIRouter(prefix="/api/outputs/v1", tags=["outputs"])
...
def get_context(request: Request, env_id: str = Query(...), business_id: UUID | None = Query(default=None)):
    try:
        ...
        return domain_error_response(
            ...
            detail=str(exc), action="outputs.context.failed",
        )
