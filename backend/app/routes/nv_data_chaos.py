...
from app.schemas.nv_data_chaos import NvContextOut
from app.services import env_context

router = APIRouter(prefix="/api/data-chaos/v1", tags=["data-chaos"])
...
def get_context(request: Request, env_id: str = Query(...), business_id: UUID | None = Query(default=None)):
    try:
        ...
        return domain_error_response(
            ...
            detail=str(exc), action="data-chaos.context.failed",
        )
