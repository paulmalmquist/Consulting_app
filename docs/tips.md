...
- **Databricks support is currently stub-only in the app data layer.** `backend/app/data/databricks_source.py` raises `NotImplementedError`; do not claim live Databricks execution without a Databricks CLI/job receipt. Use a Databricks-ready notebook plus local fallback until a real workspace run is proven.
