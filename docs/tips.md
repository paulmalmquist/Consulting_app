...
**Live run receipt (2026-06-25) — table created, data landing gated on Kafka creds:**
- **Databricks is touchless.** `databricks current-user me` works (there's a `~/.databrickscfg`), and `DATABRICKS_PAT` is in `backend/.env` (NOT in the worktree — `.env` is gitignored, so load it from the main checkout `C:\Projects\Consulting_app\backend\.env`). `DatabricksClient` (`skills/historyrhymes/scripts/databricks_client.py`, config `skills/historyrhymes/config/databricks.json`) runs SQL via warehouse `0e56420fb707d861` (Small, autostop 15m).
...
**Live bounded landing VERIFIED (2026-06-25) — 1279 rows in bronze at Delta v1:**
- **Creds live in the Databricks secret scope `stargate_kafka`** (`CONFLUENT_API_KEY` / `CONFLUENT_API_SECRET` / `CONFLUENT_BOOTSTRAP_SERVERS`). Store them leak-free via the Secrets REST API (`/api/2.0/secrets/put`, `string_value` in the TLS body) — NEVER `--string-value` on a shell line and never `echo`. List returns names only. The loader reads them on Databricks via `DBUtils(spark).secrets.get(scope, name)` (`_kafka_conf(spark)`); env `CONFLUENT_*` is the local fallback. Delete any local `confluent_key.json` after storing (now gitignored via `*_key.json`).
- **The local one-shot landing path** (most controllable for a bounded run): probe watermarks → `plan_offsets` (cold start) → consume the bounded ranges with `confluent_kafka` (assign explicit offsets, stop at end) → stage Parquet to the UC volume (`_volume.upload_file`) → `COPY INTO`. The Spark loader path (`stargate_bronze_loader.py --execute`) is the durable scheduled equivalent.
- **Three gotchas that cost iterations:** (1) an **all-null column** (e.g. `kafka_key`, since these messages have no key) becomes a *null-typed* Parquet field → `COPY INTO` fails `DELTA_FAILED_TO_MERGE_FIELDS`. Fix: write Parquet with an **explicit pyarrow schema**. (2) `COPY INTO` wants the **directory + `FILES=('name.parquet')`** form, not a bare file path. (3) High-throughput short-retention topics (`telemetry.v1`) **age out the cold-start tail** → "offset out of range"; clamp the consume start to the live **low** watermark (Spark equivalent: `.option("failOnDataLoss","false")`).
...
## Ticket B END-TO-END: durable consumer → stamp → backend deploy → verified flip (2026-06-25)

The full lineage chain is live in prod. Durable, reusable lessons:
- **Run the consumer locally (the Railway image lacks confluent-kafka).** Reuse the real `build_row`+`persist_row` from `telemetry_stream_consumer.py` (anomalies decode via `loads_registry_json` — JSON, NO Schema Registry needed; only telemetry.v1 is protobuf). Tenant is `env_id="telemetry-demo"`, `business_id="7e1eb000-0000-4000-a000-000000000001"` (config defaults `TELEMETRY_STREAM_ENV_ID/BUSINESS_ID`). Mint a throwaway Confluent key (`confluent api-key create --resource lkc-gqpvvyv --output json`), pipe the JSON to the runner (secret never printed), and `confluent api-key delete <id> --force` after.
- **Stamp matched-only via a temp-table join** (idempotent, no blind updates): load bronze coords into a `TEMP TABLE`, `UPDATE … FROM` on `(kafka_topic,kafka_partition,kafka_offset)`. `telemetry_app` (the `TELEMETRY_DATABASE_URL` role) HAS UPDATE on these tables — no owner cred needed. Get `delta_version`/commit-ts from `DESCRIBE HISTORY … LIMIT 1`. (`DESCRIBE DETAIL` `location` came back empty for the managed table → `databricks_path=""`; cosmetic, the layer surfaces catalog/schema/table/version.)
- **Backend prod deploy:** the running backend can be far behind main (`/api/version` = `{git_sha:null}` = an old deploy predating the `_git_sha.txt` mechanism AND the Lakebase migration). Deploy from a **clean main worktree at the merged SHA**: `git -C <wt> reset --hard origin/main`, `cd <wt>/backend && railway link -p authentic-sparkle -e production -s authentic-sparkle`, then `bash scripts/deploy_backend.sh` (writes `backend/app/_git_sha.txt`, `railway up --service authentic-sparkle`). `railway up` returns after upload; **poll `/api/version` for the new SHA** to confirm the build finished. Lakebase host is `ep-royal-pond-…databricks.com/databricks_postgres` (role `telemetry_app`).
- **VERIFY THE FLIP via the provenance-by-id route, not the rows list.** `/api/telemetry/stream/kafka/rows` is `ORDER BY ingested_at DESC` and `persist_row` uses the DB default `now()` per insert, so the LAST-consumed (highest-offset) rows are newest — if those are the unstamped tail, the newest-N shows 0 `available` even though older rows are stamped (don't panic). Hit `/api/telemetry/stream/kafka/provenance/{row_id}?env_id=…&business_id=…` for a known-stamped row id → `databricks_lake.status=available` with real catalog/schema/table/delta_version. NOTE: consumer-persisted anomaly rows have `anomaly_id=NULL` (the JSON payload had no `anomaly_id` field), so `anomaly_lineage(anomaly_id)` can't target them — use provenance-by-row-id. `verify_lineage.py`'s DEFAULT run asserts the layer STAYS not_available for an UNKNOWN anomaly (the honesty check) and PASSES — that's expected, not a regression.

## Telemetry WCAG contrast — audit before bumping (PR 5.1, 2026-06-25)

Don't eyeball contrast — measure it. A small stdlib script (relative luminance → ratio) over the `C` palette (`primitives.tsx`) against the four backgrounds (`bg/rail/panel/panelHi`) found the ONLY WCAG-AA failures were:
- **`C.faint` on panels** (4.37 / 3.96 < 4.5 for small text) — it's the label/caption/footnote/provenance/empty-state-hint/axis color used everywhere. Fixed by one token bump `#6f7e90`→`#8392a4` (panel 5.71 / panelHi 5.18; kept it BELOW `dim` #9fb0c4 so the text→dim→faint hierarchy holds). Because `RS.faint = C.faint` and rsTokens source from `C`, this one edit fixes the RS surfaces (charts/nav) too.
- **`RS.gray` (#3D4D60) as the "noise" RsChip text** (~2.1, unreadable). But `RS.gray` is ALSO the de-emphasized noise-scatter FILL (low contrast is intentional there) — so recolor just the chip (`RS.dim`), don't bump the token.

Everything else already passed AA (text 14–17, dim 7.4–8.9, all accents 5.7–13.6), so this was a 2-line change, not a sweep. No test asserts the hex literals (safe to change tokens); run the full telemetry suite to confirm no snapshot drift.

## EvidenceContract cards are bespoke, not framed (PR 3.1, 2026-06-25)

The 8 "PR-B" evidence cards (`DataCollectionCard` … `CompetenceEnvelopeCard`) + the reference cards (`RulConformalCard`, `RegimeAnomalyCard`) do NOT use the `TelemetryEvidenceCard`/`EvidenceContract` frame — they're bespoke (`PageHeading` + `Panel`/`MetricCard`/`StatGrid`/`ScrollTable`). "Field-fill" means matching the reference's **header chip-row** (`computed evidence artifact` / `as of {as_of}` / source `Tag`s) + **Method/Limitations footer**, NOT adopting the frame. The `TELEMETRY_FRONTEND_REFACTOR_INVENTORY.md` PR-B gap table is **partly stale** — verify each card before editing:
- **Already satisfied (don't touch):** `KnownAnomalyCard` wires `null_reason` (line ~56); `AiEvidenceCard` shows `answer_source`/`is_refusal`/`null_reason` per response; `DeploymentReceiptCard` has live/built tags + CI method + "description, not a live signal" boundary.
- **Genuine gaps filled:** `CompetenceEnvelopeCard` lacked its `as of {ev.as_of}` chip (the JSON had it); `DataCollectionCard`/`FeatureContractCard`/`PipelineEvidenceCard`/`ModelEvidenceCard` lacked an explicit `sourceStatus` `Tag` (only conveyed it in the blurb).
- **Honesty:** do NOT add `asOf` to live-fetch cards (their responses carry no `as_of` → it'd be fabricated; `PipelineEvidenceCard` already shows a real "last run"). Adding chrome chips never touches the frozen evidence JSON, so the PR #381 evidence-freeze guard stays green. Inline-style lint warnings on these files are the codebase-wide `C`-palette convention (not errors).
...
- **It lives under `backend/tests/` on purpose** — that's the only Python test dir CI runs (`backend-lint` → `pytest tests`). telemetry-platform's own `test_*_core.py` are NOT in CI. A test that needs CI coverage but reads repo-wide files should sit in `backend/tests` and resolve paths via `Path(__file__).resolve().parents[2]` (repo root); the full repo is checked out even though the job's working-dir is `backend/`.
- **Two-copy reality:** each evidence card reads a VERBATIM copy under `repo-b/src/lib/telemetry/*Evidence.json` (camelCase), separate from the `telemetry-platform/*_evidence.json` source. Only rul/regime/competence have frontend copies; lead_lag/analog don't. The freeze test includes a parametrized `source == frontend-copy` drift guard so the displayed numbers can't diverge from the evidence of record. If you edit one copy, edit both (or the guard fails).
- **Expected values are hardcoded literals**, not read-from-the-same-file (that would be circular). Mutating a JSON without mutating the test fails — verified: perturbing `picp_two_sided` 0.86→0.87 failed 2 tests (the freeze assert + the drift guard). Read JSON with `encoding="utf-8-sig"` so a BOM on a frontend copy doesn't break `json.loads`.

## The no_legacy_repe_reads allowlist is PER-FILE, not per-block (PR 4.3 cleanup, 2026-06-25)

When the authoritative-state lockdown migrated `re_env_portfolio.get_portfolio_kpis`, it added an early `return re_authoritative_snapshots.get_released_portfolio_kpis(...)` and left the old hand-rolled SQL (196 lines) as unreachable dead code below it. Removing that dead block is safe and *aligned* with the lockdown (it deletes a legacy REPE read path), but two non-obvious traps:

- **Don't remove the file's `SNAPSHOT_BUILDER_ALLOWLIST` entry just because you deleted one legacy block.** The allowlist in `verification/lint/no_legacy_repe_reads.py` is keyed by **file path**, and a single file can hold both a *migrated* function (reads authoritative state) and a *not-yet-migrated* one. `re_env_portfolio.py` still reads `re_fund_quarter_state` in `get_fund_comparison` (the LATERAL join), so the entry must stay. Grep the whole file for the banned tables (`re_fund_quarter_state|re_fund_metrics_qtr`) before touching the allowlist.
- **ruff won't catch a dead module-level helper.** Deleting the block orphaned the private `_row_to_str` (used only by the block); ruff passed clean because it doesn't flag unused module-level functions — only unused imports/locals. After removing a block, grep each private helper it called (`grep -c "\bname\b"` → 1 means only its own def) and remove the now-orphaned ones too.
- **Proving unreachability + no-side-effects without running prod:** the dedicated test already monkeypatches `get_cursor` to raise and asserts the live paths never query the DB — that test passing *is* the proof the SQL block is dead. `test_state_lock_invariants.py` runs the lint, so a green `pytest` on it confirms the lockdown still holds after the deletion.

## Promoting deptry DEP003 (transitive-but-imported) deps — use `>=` floors, not `==` (2026-06-25)

deptry `DEP003` = a package imported directly in the code but only present *transitively* (no line in `requirements.txt`). The fix is to declare it explicitly. Conventions for this repo (`backend/requirements.txt` already does this for `cryptography`, `google-auth`, `confluent-kafka`):
...
**Per-package judgment matters (second DEP003 batch, 2026-06-26):** not every DEP003 should be pinned.
- **Skip a package that isn't even installed.** `vertexai` was DEP003 but `importlib.metadata.version('vertexai')` raised `PackageNotFoundError` — it's a lazy, optional, heavy cloud SDK. Pinning it would *force-install* it and change the footprint. Left transitive on purpose; documented the skip.
- **Framework-coupled internals: floor at the framework's own floor.** `starlette` is imported directly (`BaseHTTPMiddleware`) but is FastAPI's tightly-coupled core. FastAPI 0.115.6 declares `starlette>=0.40,<0.42`. Pinning `starlette>=0.40` (= FastAPI's own floor, no upper bound) is provably conflict-proof: it never restricts below what FastAPI already requires, and never caps above it. Check the framework's range first: `python -c "import importlib.metadata as m; print([r for r in m.requires('fastapi') if 'starlette' in r])"`.
- **Heavy multi-puller data deps: conservative low floors, not floor-at-resolved.** `pyarrow` (resolved 24.0.0, pulled by bigquery/pandas/sklearn) and `scipy` (resolved 1.17.1, pulled by sklearn/pandas) got `>=14` / `>=1.10` — well below resolved. Floor-at-resolved risks *forcing a CI upgrade* when CI's pinned pullers (here `scikit-learn==1.6.1`) resolve a different version than your local env. A low floor clears DEP003 just as well (clearance only needs the dep *declared*, the floor value is irrelevant to it) with zero forced-upgrade risk. Record the resolved version in the comment.
...
## Telemetry drill-through / export framework (Phase 8 PR 8A, 2026-06-26)

- **The `/api/telemetry/[...path]` proxy corrupts binary downloads by default.** It reads upstream with `await upstream.text()` (`api/telemetry/[...path]/route.ts`), which mangles non-UTF8 bytes. For an XLSX (or any binary) download, add a branch that forwards `await upstream.arrayBuffer()` + the `Content-Disposition` header when the content-type is a download type; leave the existing `.text()` JSON path untouched. Without this the .xlsx arrives unopenable.
- **Make a server export safe by reusing an existing scoped read service, not new SQL.** `telemetry_export.py` keys a fixed dataset allowlist to existing `telemetry_serving` functions (already tenant-scoped). The dataset name is a dict lookup (unknown → 404 + null_reason), columns are a fixed per-dataset tuple, the row count is bounded — so there is **no request-controlled identifier in any query**. Bonus: the export provably equals the page data (same service). Test allowed/denied/limit/cap/empty/file-shape by calling the route function directly with the service monkeypatched (no DB, no TestClient).
- **For an in-memory file, return `Response(content=bytes, media_type=…)`, not `StreamingResponse(BytesIO)`.** `StreamingResponse` wraps the iterator in a threadpool async-iterator, so `resp.body` isn't readable in a sync test; plain `Response` exposes `.body` (the bytes) — `load_workbook(io.BytesIO(resp.body))` verifies the shape.
- **Source-kind honesty framework:** every drilled metric / export carries `live-rows | computed-artifact | fixture | unavailable`. `live-rows` (real `tel_*` via an existing API) drills to rows + CSV/XLSX; `computed-artifact` (e.g. RUL FD001 evidence) and `fixture` (Flight Readiness `public/labs/factory-ml/*.json`) drill to the artifact, labeled "not live serving" / "committed fixture"; `unavailable` renders null_reason + disabled export. Never an empty/fake file or a fabricated link — the `ExportTo*Button`s disable-with-reason and `factoryEvidenceLinks` builders return `href=null + unavailableReason` when an id is missing.
- **Concurrency with the agent that owns `repo-b/src/components/telemetry/**`:** put new primitives in a fresh subdir (`drill/`) — new files don't conflict — and keep wiring edits to existing pages tiny + additive. Mirror, don't import, a private symbol (the factory drawer's `CopyChip`/`EvidenceLinkButton` are file-private; the shared `drill/evidenceLinks.tsx` re-implements them over the same public `factoryEvidenceLinks` builders). `git rebase origin/main` before opening the PR; the GitHub PR flow is race-safe.
- **Inline-style lint warnings from the IDE are not enforced by `next lint`** — the whole telemetry tree uses the `C`-palette inline-style idiom and passes CI. Match the surrounding code; don't extract a stylesheet for one component.
- **Verify a binary download end-to-end through the proxy, not just the backend unit.** After deploying the XLSX route, `curl` it through `novendor.ai` (header `x-telemetry-route-env-id: telemetry-demo`), save to a file, and `openpyxl.load_workbook()` it — that proves the proxy's `arrayBuffer` branch didn't corrupt the bytes. Confirmed: 200, `content-type spreadsheetml`, `attachment` disposition, loadable, real rows. The proxy forwards `Content-Type` + `Content-Disposition` but NOT the custom `X-Telemetry-*` headers (informational only — fine).
- **Backend deploy flow (no worktree stays Railway-linked).** Deploy current `main` from a clean dedicated worktree: bring it to `origin/main` (`git merge --ff-only`), `cd backend && railway link -p authentic-sparkle -s authentic-sparkle -e production` (non-interactive flags — interactive `railway link` hangs the Bash tool), then `scripts/deploy_backend.sh`. `railway up` returns after upload (build runs server-side); confirm live by polling `/api/version` for the SHA flip — that's the real signal, not the CLI exit. The script captures HEAD into `backend/app/_git_sha.txt` and marks `-dirty` if the tree has edits, so a clean checkout matters.

## Telemetry Overview polish (Phase 8 PR 8B, 2026-06-26)

- **Faint true-scale recharts axis without crowding the gutters.** To make the green commercial-share underlay read 0/50/100% on a true scale while staying subordinate: keep the `<YAxis yAxisId="share" hide domain={[0,100]}>` (hidden, just for scaling) and draw faint labeled `<ReferenceLine yAxisId="share" y={0|50|100}>` with `label={{ position: "insideLeft" }}`. A true [0,100] domain means data that only reaches ~70% sits at ~70% height naturally — no rescale — and low `fillOpacity` + painting it before the bars keeps it behind them.
- **A `null_reason` legitimately renders in several places** (the empty state *and* each disabled export button's reason). That's good UX, but `getByText(/reason/)` then throws "Found multiple" — use `getAllByText(...).length` in the test, not `getByText`.
- **Add per-event narrative without touching the type or the out-of-scope record component.** A separate `EVENT_NARRATIVE: Record<id, {...}>` in `data.ts` adds fields (harder-question, downstream link) without extending `LaunchEvent` in `types.ts` or editing `EventRecord.tsx`. Render only the fields the existing record doesn't already show (the record already prints bottleneck-solved/created as relay cells) — otherwise you duplicate text and collide with the record's `getByText` assertions.
- **Renaming a user-facing label breaks the test that asserts it.** Changing the bridge card "Resume Evidence"→"Evidence" failed `TelemetryOverview.test`. Grep tests for the old label string before renaming.

## Telemetry streaming controls + SSE export (Phase 8 PR 8C, 2026-06-26)

- **"Controls cleanup" often finds nothing to clean.** Two read-only Explore passes confirmed the Stargate/Mission Control/Control-Tower surfaces had **no duplicate controls** and were already honestly labeled — the only real change was `Hold`→`Pause`. The actual value was the **exports**, not the relabel. Inventory before assuming a cleanup exists.
- **Real rows over a recorded-capture replay is its own honesty case** — not "live", not "fixture". The Stargate anomalies/DLQ are genuine SSE-pushed rows, but the *stream* is a recorded capture. Label them "real SSE rows over a recorded-capture replay" (the page already says "recorded capture, not a live printer"); never imply live printer hardware.
- **Flatten nested SSE rows in a small pure helper, test it without rendering.** `stargate/streamExportRows.ts` maps `AnomalyRow` (nested `rule`/`scorer`/`routing`/`provenance`) to flat CSV columns with `?.field ?? null` (absent enrichment → null, **never fabricated**). A pure flattener is trivially unit-tested; the component test just checks the export button enabled/disabled.
- **Client-side `CircularBuffer` rows (`.toArray()`) → CSV only.** No `tel_*` table means no server XLSX; that's honest. Durable tables (`tel_anomaly_events` via `/stream/live`) *can* get a server XLSX dataset — but batch those backend additions into one deploy (deferred to 8D) rather than deploying mid-frontend-PR.

## Explore agents read the PRIMARY working dir, which may be a stale branch (Phase 8 PR 8D-1, 2026-06-26)

The Explore/Agent subagents run in `c:/Projects/Consulting_app` — the primary checkout, which in this session sits on a long-lived feature branch (`skill/confluent-stargate-lifecycle`), **not** `main`. So an inventory of a file changed in a recent merged PR can be **wrong**: the 8D-1 Explore agent reported "ModelPerformance has no drill drawer and there's no model_runs export dataset," but PR 8A had already added both on `main`. **Before implementing against an Explore inventory of a recently-touched file, re-read the current `main` state in your worktree** (`cons_wt_exec` off `origin/main`). The agent's findings on *unchanged* files (e.g. `RegistryConsole.tsx`) were accurate; only the recently-merged surfaces were stale.
- **Per-model fail-closed link pattern:** a `tel_model_runs` row carries `mlflow_run_id` (→ live `DatabricksRunLink`) but its `model_name` is a seed key, not a 3-part UC path (→ `ModelArtifactLink` fails closed with "seed registry key" + a copy chip). Render both per model; never fabricate the UC link. For fields that simply don't exist in the data (`training_window`/`validation_method`/`operational_use`), render an explicit "not recorded in tel_model_runs" null_reason, not a blank cell.
- **Reuse an existing XLSX dataset across surfaces that read the same table (8D-2).** Model Registry and Model Performance both read `tel_model_runs`, so Registry's "Models XLSX" reuses the 8A `model_runs` dataset (already deployed) via `exportXlsxUrl("model_runs", …)` — no new backend, no deploy. Honest because it's the same rows; label it so. Drift (`tel_drift_metrics`, already on the page) gets a client CSV from the displayed series rather than a server dataset.

## A `sys.modules` monkeypatch stub is bypassed in the full pytest suite (8D-3)

A test that does `monkeypatch.setitem(sys.modules, "app.services.telemetry_stream_etl", stub)` to fake a function **passes in isolation but fails in the full suite.** `from app.services import telemetry_stream_etl` binds the **package attribute** `app.services.telemetry_stream_etl`, which is already set to the REAL module once any earlier test imports it — so the stub in `sys.modules` is ignored, the real function runs, and (here) the closed test DB pool raised `psycopg_pool.PoolClosed`. **Fix:** import the real module in the test and `monkeypatch.setattr(real_module, "fn", fake)` — patching the attribute on the live module object works regardless of import order. (CI runs the *full* suite; "passes locally in isolation" is not enough for a module-stub mock.)
- **Reuse a heavy read service as an export extractor when it returns the exact screen data.** The `anomaly_events` XLSX dataset's extractor calls `telemetry_stream_etl.stream_live` (the same scoped/bounded query Mission Control polls) → the export equals the page, with **no new SQL** and no request-controlled table. The extra work (channels/diagnostics) is fine for an on-demand button. An empty live window → a valid **header-only** workbook + an honest `X-Telemetry-Null-Reason`, never a fabricated row.

## Artifact-backed surfaces get CSV-only export; don't force a server XLSX (8E)

When a page's data is a **committed evidence artifact** (a frontend const / evidence JSON), not a `tel_*` table — e.g. RUL Calibration's `calibrationEvidence.ts` (FD001 conformal scalars + a replay-fixture trajectory) — it gets **CSV-only** export labeled `kind="fixture"`/computed-artifact; the server-XLSX button is honestly **disabled** (there's no `tel_*` dataset to allowlist). Don't invent a backend dataset for committed-artifact data.
- **Derive honest per-point fields from the displayed values; null_reason the rest.** The unit-level drill adds `covered_80`/`covered_90` (true within the shown band) — derived, not fabricated. Fields that are genuinely a different aggregate (the 100-unit gate-flip) are marked **not-applicable here** with a pointer, not invented per row.
- **Don't conflate two RUL surfaces.** `RulCalibration.tsx` is the **CNN-LSTM FD001** page (PICP 0.778/0.903, RMSE 17.33). The **PICP 0.86 / 15-of-100** numbers are the **Spin-3 `RulConformalCard` on the Evidence page** — a different surface. Preserve each set on its own page; touching one doesn't touch the other.

## There are two "factory" telemetry surfaces — don't conflate them (8F)

`/telemetry/factory` → `FactoryNcrIntelligence.tsx` (RS-themed: `RS`/`RsPanel`/`RsChip`; **API-backed** NCR via `/api/telemetry/ncr`). `/telemetry/factory-ml` → `FactoryMlConsole.tsx` (C-palette; **fixture-backed** readiness + heatmap from `public/labs/factory-ml/*.json`). The 8A `ExportToCsvButton`/`MetricInspectorDrawer`/`DatabricksRunLink` (C-palette) drop cleanly into the RS-themed page too — the factory-ml console already mixes them, so it's precedented, not a violation.
- **API-backed but not live = `computed-artifact`, not `live-rows`.** NCR is a real Databricks **batch** run (provenance `databricks`), not live serving → label it `SOURCE_KIND_TAG["computed-artifact"]` ("computed artifact"); fall back to `fixture` when provenance is `local_fallback`. "Real rows" ≠ "live".
- **Heat-map keep/demote call:** keep it when it carries a real argument (the SCN-005 golden-pair *rhyme* is the whole point of similarity-based review) — but then state exactly what a cell means (rolling-std z-score; the ±2.5 is a *color* clamp, export the real z) and make it drillable/exportable. Demote/replace only when it's noise. Never fabricate "more convincing" cells.
- **Unpublished rollup weights → null_reason, not invention.** The readiness gauge is an opaque gold rollup; show the ingredient **counts** and state `weight_not_in_rollup` rather than back-fitting per-ingredient weights.
- **Only add filters the grain supports.** NCR exemplars carry `severity` → a severity filter is honest; they carry no per-record status/timestamp → don't add those filters, null_reason them in the drill instead.

## A live-rows table can still have no run links — drill the grain, null_reason the rest (8G)

`tel_test_runs` (the Test Runs page) is **live-rows** but the row grain is an *ingest* record — `{id, run_key, dataset, unit_or_channel, spacecraft, row_count, ingest_at, status, created_at}`, with **no** mlflow/Databricks/model-artifact/evidence fields. "Live rows" doesn't imply "has run URLs." The honest drill: expose what's there, **fail-close** run-type/model-kind/MLflow-Databricks-run/model-evidence-artifact with a null_reason ("ingest window, not a training/scoring run"), surface the `run_key` as a **copyable id**, and **never fabricate a run URL** (the rule: id-without-resolvable-URL → copyable id + unavailable, not a fake link). CSV over the displayed fields is the honest export; no server XLSX for operational ingest rows.
- **Inventory before assuming a surface is missing work.** 8G-2's "Run Autopsy/Review" was already met by the existing `ReplayForensicsDrawer` (source-kind honesty, fixture-vs-live run-id mismatch *surfaced not reconciled*, MLflow/Delta links with `NaRow` fail-closed, confusion-matrix evidence, full lineage). Route-first inventory turned a planned PR into a no-op — don't force a marginal CSV into a delicate 500-line honesty surface. And `RunAutopsy` (data-engineering, hidden-but-resolving) is out of the telemetry demo scope — don't revive a hidden page into nav to "complete" a phase.

## A rationalization phase may already be done + test-locked — inventory before editing nav (8H)

The telemetry nav was already hide-before-deleted in earlier phases, and **`telemetryNav.test.ts` locks it**: it asserts the 6 visible sections + exact slugs, that `governance`/`how-it-works`/`evidence`/`copilot`/`data-engineering*` are NOT in nav, and that the hidden routes still resolve via `telemetryHref`. So "8H — page rationalization" had **no low-risk code left**: no stale bridges to repoint, content already folded (resume/how-it-works → Overview + per-page `EvidenceContract`; lineage → Metric Lineage; monitoring/spike → System Health; DE → hidden section + the portable ADE link), and the only deletion candidates (`data-engineering/{workbench,workflows,sources}`) stay hidden-but-resolving pending explicit quarantine approval.
- **Grep bridges from VISIBLE pages only.** Most matches for a hidden slug are noise: intra-hidden-page links (a hidden DE page linking its own subpages), test strings, or **palette/status literals** (`s === "evidence"` in `TelemetryArchitectureMap` is a color status, not an href). Verify each before "fixing" it.
- **Nav removals need owner sign-off — keep-visible is the safe default.** Hiding `control-tower` (the sole Agent Operations item) or dropping the ADE "Platform" cross-link changes the rail and is exactly what hide-before-delete gates. Surface these as decisions; don't unilaterally change a test-locked nav.
- **"Delete if it doesn't break anything" = delete only the orphan.** Of `workbench`/`workflows`/`sources`, only `workflows` had zero inbound links (verified: nothing imports `WorkflowTemplates` but its own page+test; `DataEngineeringOverview` links the other two via `ModeCard` + stat grid). Delete the orphan; leave the wired ones hidden-but-resolving. Prove no breakage by *typecheck* (a dangling import surfaces instantly) + the suite count dropping by exactly the deleted test file's tests.
...
## Enrich a streaming surface by surfacing data it already computes; warm it in the browser, not headlessly (Stargate)

The Stargate console looked "done" but had only CSV export from 8C while the other pages got the full 8D-8G drill/export/source-kind pass. The enrichment was almost entirely **surfacing data the bridge already streamed but the UI dropped**: the `AggRow` in-flight window + `BridgeHealth` (engine/freshness/uptime) became a `StreamContextStrip`; the DLQ ticker (which already carried topic/reason/bytes/preview) got an inspection drawer to match the anomaly one; the raw `TelemetryPoint` series behind the chart got a CSV. No new data, no backend — just stop dropping what is already on the frame. Every added field **fails closed** ("not available — <reason>", never a zero that reads as real).
- **These surfaces warm in a live browser, not via a headless POST.** The Stargate SSE buffer is client-side (fills when a browser connects to the bridge during a recorded-capture replay), and `/stargate/replay/cycle` 404s through the Vercel proxy — so "seed it" for a demo means opening the page and clicking "Start recorded capture", not a server call. Mission Control's `tel_anomaly_events` *is* server-persisted, but its worker being `already_running` with `events=0` means the upstream Confluent/Stargate→anomaly spine is cold (standing it up has real cost — out of scope for a UI enrichment). The honest move is to ship the fail-closed UI and warm the live rows at demo time, not to fabricate rows or spin up paid infra to fill a screenshot.
## A new operational domain rides the proven serving architecture, not a toy fork (Relativity MES Sandbox / Phase 10)

A synthetic domain (`rel_*` MES/ERP/PLM facsimile) plugs into the *same* data-product stack as telemetry: deterministic generator → Databricks medallion → Postgres/Lakebase serving → FastAPI → Next.js dashboards. "Synthetic" is about data privacy/honesty, never about toy architecture. Hard-won specifics:

- **One deterministic generator is the source of truth.** `scripts/relativity_mes_seed/` builds the dataset once and emits *both* the Postgres source/serving migrations AND the Databricks bronze/gold load SQL, so fixtures and Lakebase can never drift. Put demo "constants" (e.g. NCR rework cost) into a real source row (`rel_erp_prod_order_cost cost_element='rework'`) so gold is fully *derivable from source* — otherwise the medallion can't reproduce gold and the lineage claim is hollow.
- **Don't render from local JSON when a real serving path exists.** Dashboards call the FastAPI routes (`apiFetch`), which read the Lakebase serving tables and **fail closed with `null_reason`** (no fabricated fallback). The `serving_provenance` column (`seed-bootstrap` → `databricks-gold`) keeps the source-kind chip honest about which load is live.
- **Owner-created Lakebase tables are NOT covered by default privileges.** `telemetry_app` (the backend role) `has_table_privilege(...)='false'` on brand-new `rel_*` tables even though it reads `tel_*` fine. Bake a guarded `GRANT SELECT ... TO telemetry_app` (wrapped in `IF EXISTS (SELECT 1 FROM pg_roles ...)` so CI/local no-op) into the migration. Verified the gap directly via `pg`.
- **Lakebase apply recipe (no PAT, no password):** the credential CLI is `databricks database generate-database-credential --json '{"instance_names":["novendor-telemetry"]}' -o json` (NOT `--instance`; `request_id` is optional); user = the Databricks email (`%40`-encode the `@`), db `databricks_postgres`, host `ep-royal-pond-…`. `node apply.js --dry-run` validates parse offline first. **The minted token is SHORT-LIVED (~1h, see `expiration_time` in the response) — every refresh needs a FRESH mint right before `apply.js`; a token reused from a prior session fails auth with `28P01`.** To re-run an already-numbered, idempotent migration (DELETE-by-`ingest_batch_id` + INSERT) against the live serving, call `node repo-b/db/schema/apply.js --files 10037,10038 --no-single-transaction` — `apply.js` runs the named files directly, it has no applied-migration tracker, so this is the way to push a regenerated `emit` seed to Lakebase.
- **Reuse the whole telemetry primitive kit** — don't rebuild it. `MetricInspectorDrawer` (drawerPrimitives), the `drill/` module (`SourceRowsTable`, `ExportToCsvButton`, `SOURCE_KIND_*`), `evidenceCard` source chips, `useFactoryMlData` loader pattern, the `C` tokens. Adding a nav section = one `TelemetryNavGroup` union member + one `TELEMETRY_NAV` block + one `TELEMETRY_NAV_GROUPS` entry + one `GROUP_ACCENT` color (it's a `Record`, so a missing key is a typecheck error — that's the reminder).
- **Databricks warehouse can fail platform-side.** With no `claude_token.txt`, feed the SDK client an OAuth bearer from `databricks auth token` (`.access_token`) as `DATABRICKS_PAT`. But the serverless warehouse here reported `health=FAILED — "Clusters are failing to launch"` and never reached RUNNING — a manual-platform-action blocker, not a code defect. Ship the medallion as committed runnable code, serve from the proven Lakebase route, and say so plainly in the README + plan. Always `databricks warehouses stop` after — a half-started warehouse still accrues cost.
- **Test-assertion gotcha:** demo values that legitimately appear twice (a vehicle serial in both the table link and the "highest-risk" card; a lot in both the tree row and the where-used chip) break `findByText` (multiple-match). Use `findAllByText(...).length >= 1`. Text split across sibling `<span>`s won't match `findByText(/whole sentence/)` — assert the single-node substring instead.

## Auditable historical-ML evidence: surface stored fields, fail-closed the rest, and sync the freeze source

Building the "Historical ML Evidence Bridge" (Stargate → regime-conditioned PCA on NASA C-MAPSS FD004):
- **Make every drawer field a STORED, grounded JSON field, not component-invented copy.** The model card, the "why this works" theory, the source query, and the lineage chain all live in `regimeAnomalyEvidence.json` as additive blocks. That keeps the displayed copy auditable (it IS the evidence of record) and satisfies "explanatory copy grounded in stored evidence."
- **Real lineage already exists in the metadata catalog.** `backend/app/data/telemetry/metadata_catalog.json` carries the committed chain `src_nasa_cmapss → bronze_cmapss → silver_cmapss` (ids, schema, asset_path, `status: unknown`). Surface that instead of inventing lineage; its `status: unknown` is honest ("committed definition, live materialization not probed").
- **Databricks links are already fail-closed.** `lib/lab/factoryEvidenceLinks.ts` `deltaTableLink("catalog.schema.table")` returns a live `<a>` to the committed workspace (`dbc-2504bec5-b5ab`, override via `NEXT_PUBLIC_DATABRICKS_WORKSPACE_URL`) or `{href:null, unavailableReason, copyText}`. Reuse it; never fabricate a workspace URL. Render unresolved links as "not available — <reason>" + a copyable id.
- **Fail-closed the metadata the artifact genuinely lacks.** Full table rowcount, live materialized-at, and live materialization status are not in the committed artifact → store them as `null` + a `*_null_reason` and render "Not available — <reason>". Show rows-USED (fit/eval) which ARE real, not a fabricated table rowcount.
- **The evidence freeze guard requires the SOURCE copy to match.** `backend/tests/test_evidence_freeze.py::test_frontend_copy_matches_source` asserts the repo-b frontend JSON is value-identical to its `telemetry-platform/` source. Editing only the frontend copy fails the guard — `cp` the updated frontend JSON over the `telemetry-platform/` source so both load-equal (the per-value `test_*_frozen` tests still guard the actual numbers). Additive fields are fine as long as both copies carry them.
- **Bridge framing must not overclaim.** The Stargate bridge card + the drawer caveat both state the live detector is a transparent rolling-MAD baseline, NOT the PCA model scoring live. Surfacing a validated historical pipeline next to a live baseline is honest only if you say which is which.

## NEXT_PUBLIC env: an empty string is "unset" and can hide a whole page; set it via the REST API on Windows

The Stargate console gated its entire render on `bridgeBaseUrl()` (reads `NEXT_PUBLIC_STARGATE_BRIDGE_URL`). The Vercel prod var was present but set to `""` for ~14 days — `""` is falsy, so `configured=false` and the `!configured` early return hid everything, including the static Historical ML Evidence Bridge. Two lessons:
- **Don't gate static/auditable surfaces behind a live-connection env.** The historical ML evidence reads a committed artifact; it now renders in BOTH the configured and not-configured branches (a shared `mlEvidenceBridge` fragment), so a missing/empty live-bridge URL never hides it. Gate only the live SSE stream on the live URL.
...
## Telemetry Overview hero adapts to the Bottleneck Map node (2026-06-27)

Consolidated the launch-data Overview so the hero IS the node story (the map is just the selector). Reusable facts for the next session on `repo-b/src/components/telemetry/`:

- **Where the launch/node data lives:** `context/BottleneckMap/data.ts` — `EVENTS: LaunchEvent[]` (typed in `types.ts`) plus `EVENT_NARRATIVE` (the per-event `harderQuestion` + `provesNextSlug/Label` CTA). 100% static public data, **no API**; the comment is explicit ("stays in code, never in Supabase"). `EVENTS_CHRONO` is the presenter order; `INNOVATION`/`FUNDING` resolve the `dimLabel` per `colorBy`. Don't invent KPI/caption values — they're all on the event.
- **Selection is lifted to the page.** `TelemetryOverview` owns `selectedId` (default `null`) + a resolved `selectedEvent` (surfaced by the map via `onSelectedEventChange`). The hero branches on **`selectedEvent`, never `selectedId`** — presenter mode populates `selectedEvent` while `selectedId` stays null. `BottleneckMap` is now a **controlled selector**: `selectedId` / `onSelectNode` props, hybrid with an uncontrolled `terran1` default for standalone use (`isControlled = onSelectNode !== undefined`).
- **`resetSignal` is the "stop everything" command channel.** The map owns `presenterIdx`, so a parent "Back to overview" can't just null the event — the map would re-emit the presenter step. Bump a `resetSignal: number` prop; the map's effect calls `clearAll()` (stops presenter + clears selection) only when the value actually changes (tracked via a `useRef`), then emits `null` exactly once. Reuse this pattern any time a parent must reset child-owned ephemeral state.
...
## Real medallion on Dataproc Serverless PySpark over BigQuery (relativity_mes, 2026-06-27)

Turned the cosmetic `relativity_mes` BigQuery medallion (silver = `SELECT * FROM bronze WHERE synthetic IS TRUE` views, gold = Python literals) into a real one: ugly bronze → Dataproc PySpark silver → Dataproc PySpark gold → Lakebase serving (`serving_provenance='dataproc-gold'`). Lessons for future GCP/Spark sessions:

- **bq CLI + Dataproc both need an explicit Python here.** The bundled shim calls a missing `python3.14`; prefix every `gcloud dataproc`/`bq` call with `export CLOUDSDK_PYTHON=/c/Python314/python`. The Dataproc batch runtime itself uses its own conda Python — unaffected.
- **Dataproc Serverless is the honest "PySpark on GCP" answer and it just works.** `gcloud dataproc batches submit pyspark gs://.../job.py --region=us-central1 --deps-bucket=gs://<bucket> --version=2.2 -- <args>`. Default compute SA (`roles/editor`) covered BQ read+write and GCS deps with zero IAM fiddling. Colocate the GCS bucket with the BQ dataset region (the dataset was US multi-region → US bucket, no cross-region cost).
- **Read works out of the box; the WRITE path needs the indirect connector options.** `df.write.format("bigquery").option("temporaryGcsBucket", BUCKET).option("writeMethod","indirect").mode("overwrite")`. Reads need nothing extra. Prove the write path on the first real job, not a read-only smoke test.
- **Spark `cast()` behaves like SAFE_CAST** (bad input → null), so it's the clean way to type a string landing zone. `to_timestamp`/`to_date` likewise null-on-failure. This is what makes "silver casts types" demonstrable: land bronze as ALL-STRING, cast in silver.
- **Two recurring Spark join footguns, both fatal at batch runtime not locally:** (1) `df.select("*", expr.alias("col"))` to "replace" a column actually *appends a duplicate* → `AMBIGUOUS_REFERENCE`; use `withColumn("col", expr)` instead. (2) After `.alias()`-ing join keys, a `df.join(other, "key", ...)` USING-clause fails (`UNRESOLVED_USING_COLUMN`); use an explicit `col("a")==col("b")` condition. Give every joined-in column a distinct name up front.
- **Left-joining an enrichment table that isn't unique on the join key fans out the left rows.** Genealogy gold went 106→107 because a node carried multiple NCRs. The generator's dict (`ncr_by_node[node]`, last-write-wins) hid this; in Spark you must dedup the right side first (`row_number() over partitionBy(node)`), then the row count matches silver. Always count-check gold vs silver after a join.
- **Don't inject mess into a test-locked generator — add a separate uglifier.** The seed generator feeds the Lakebase source migrations and has identifier/invariant tests. A standalone `load_ugly_bronze.py` that takes the clean rows and writes dirty, all-STRING bronze keeps the generator's contract intact while making bronze realistically dirty. Make the mess deterministic (seed-derived) and protect the invariant rows from corruption so they survive into silver.
- **Test the silver vocab covers the uglifier's outputs.** A pure-Python test asserts every synonym/casing the uglifier can emit is a key in the silver job's `VOCAB`, so no valid row is wrongly quarantined as out-of-domain. To load the Dataproc job modules in CI without pyspark installed, stub `pyspark`, `pyspark.sql`, `pyspark.sql.types` in `sys.modules` before import (the jobs only *use* Spark inside `main()`; module-level constants are plain data).
- **Lakebase write from a script:** mint a short-lived cred with `databricks database generate-database-credential --request-id <uuid> --json '{"instance_names":["novendor-telemetry"]}'` (note: `--json`, NOT `--instance-names`), connect psycopg2 to `ep-royal-pond-d2wqvv3i.database.us-east-1.cloud.databricks.com:5432/databricks_postgres` as `paulmalmquist@gmail.com`, and **`SET app.env_id='telemetry-demo'` first** or RLS silently rejects the INSERTs. Tokens expire ~1h — mint inside the same command that applies.
- **Dropping BQ views/tables: use the Python client, not `bq rm`.** `bq rm -f -t` silently no-op'd here; `bigquery.Client().delete_table(..., not_found_ok=True)` in a loop worked. Multi-statement `DROP VIEW ...;` via `bq query` also choked on backtick quoting — one object per `delete_table` call is reliable.
- **New serving_provenance value needs frontend recognition.** Flipping serving to `dataproc-gold` left the Lineage page treating it as non-live until `LineageSourceConsole.tsx` + `relativityMes.ts` learned the new value (they only knew `bigquery-gold`/`databricks-gold`). When you add a provenance label, grep the frontend for the existing labels and add the new one everywhere (tone, medallion name, prov text, servingLabel) + a test case.
...
## RUL Calibration evidence surface (2026-06-29)

Redesign of `/lab/env/[envId]/telemetry/calibration` into an inspectable evidence surface. Reusable lessons:

- **One drawer, a nested discriminated union — never spread a payload that owns `kind`.** `RulArtifactStep`/`RulMetricEvidence`/`RulReliabilityBin` each carry their own `kind`/`id`/`status` fields. Spreading them next to a drawer discriminant (`{ kind: "artifact", ...step }`) makes `step.kind` overwrite the discriminant and TS collapses the union to `never`. Nest the payload instead: `{ kind: "artifact"; artifact: RulArtifactStep }`. One `useState<RulDrawerTarget|null>` then drives a single drawer for every drill-through.
- **Make the chart-click a pure helper, not an event handler.** `buildRulChartPointDrawerTarget(point)` turns a `CalibrationPoint` into the drawer target (error/absError/containment/lateRisk/provenance). The chart's recharts `onClick`/tooltip both call it. jsdom can't drive recharts hover, so the test exercises the helper directly — the interaction is covered without simulating SVG events.
- **recharts interval bands = range `Area`.** For an 80/90% band, add `band80: [lo, hi]` / `band90: [lo, hi]` tuple fields to each row and render `<Area dataKey="band90" .../>` then `<Area dataKey="band80" .../>` (faint→stronger fill), with `<Line>`s for the point series on top. Simpler and more honest than stacked lower+width areas. Matches `stargate/TempVibrationChart` (the house recharts pattern) — `ReferenceArea` for the late-risk zone, custom `<Tooltip content=…>` for rich hover.
- **No shared Tooltip primitive exists; don't add a dep.** `@radix-ui/react-tooltip` is NOT in repo-b (only `react-dialog`). Build an accessible CSS/focus tooltip: a real `<button>` trigger (keyboard-focusable), show on hover+focus, `aria-describedby` to the bubble, Escape to dismiss. Keep critical caveats on the page too — never tooltip-only.
...
- **All four Vercel projects are real** (verified `vercel project ls` 2026-07-02): `consulting-app` = the live frontend (novendor.ai, rootDir repo-b, auto-deploys from main); `repo-b` (no production URL); `backend` legacy/empty; `floyorker`. **Correction (2026-07-03): production secrets live on `consulting-app`, not `repo-b` — `repo-b`'s env store is empty** (`vercel env ls` on it returns "No Environment Variables"). CLAUDE.md's "backend secrets live on the repo-b project" credentials-flow note is stale; pull from `consulting-app`.

## Credential rotation via Vercel — the CLI stdin trap (2026-07-03, Story #758)

- **`vercel env add VAR production` piped from stdin silently stores an EMPTY value in this environment, while still printing `Added Environment Variable` (a false success).** Confirmed across `vercel env add < file`, `cat file | vercel env add`, `printf '%s' "$V" | vercel env add`, and Python `subprocess(shell=True)` (which on Windows runs cmd.exe, not git-bash, so the pipe never reaches vercel at all). A dummy `printf` add once showed "Added" but round-trip pull proved the stored value was empty. **Always round-trip verify** (`vercel env pull` → compare) after any `env add`; never trust the "Added" line.
- **Reliable path: the Vercel REST API.** `POST https://api.vercel.com/v10/projects/{projectId}/env?teamId={teamId}` with `{key,value,type:"encrypted",target:["production"]}`; delete existing first via `GET /v9/.../env` + `DELETE /v9/.../env/{id}`. Token is in `~/AppData/Roaming/com.vercel.cli/Data/auth.json` (`token` key); ids in `.vercel/project.json` (`projectId`, `orgId`=teamId). Deterministic — value in the JSON body, no stdin games.
- **Env vars bind to a deployment at creation.** Changing a project var does NOT affect the running deployment — you must `vercel redeploy` (then it re-aliases novendor.ai) for serverless functions to see the new value. Verify live, not just via `env pull`.
- **Rotate every target.** `ADMIN_INVITE_CODE` existed on `production`, `preview`, AND `development` — a leaked value stays live in preview/dev until all three are rotated.
- **Never regenerate-and-discard before the destination is confirmed.** Early attempts rotated the Supabase password to a random value, wrote it to a scratch file, set Vercel (which silently stored empty), then deleted the scratch file — losing the only copy while Vercel was wrong. Hold the value in-process until the destination round-trips clean, then delete.
- **The Vercel-stored `SUPABASE_SERVICE_ROLE_KEY` was stale (401).** Fetch current Supabase keys from the CLI (`supabase projects api-keys --project-ref … -o json`), not from the pulled Vercel env, for admin-API work.

## Clean-tree hygiene on the shared checkout (2026-07-06, Story #762)

- **The shared checkout `c:/Projects/Consulting_app` is a stale detached-HEAD working dir** (was 0 ahead / 10 behind origin/main) shared by ~10 concurrent worktrees. Its `M` files (`CLAUDE.md`, `SKILLS_INDEX.md`, `docs/tips.md`) were an unrelated in-flight workstream (a `gke-alive-demo` skill registration) on an OLD base — and `SKILLS_INDEX.md`/`CLAUDE.md` had already been regenerated differently on main by PR #500. **Never stage or "fix" those `M` files as part of an unrelated ticket**; they'll clobber merged work or resurrect an obsolete format. Do clean-tree gitignore/doc changes in a fresh worktree off `origin/main` and PR them; only apply in-place, non-committing fixes (a bug revert, ignore protection) to the shared checkout itself.
...
## Relay planning-run lessons (2026-07-08)

- **Bundle-judgeable acceptance criteria reach PASS; provenance criteria force MAX_ITER.** The sustainability planning run passed all 17 criteria on iteration 1 because every criterion named something the reviewer could confirm from the diff ("the produced plan names X", "the plan's section states Y"). The two prior live runs hit MAX_ITER because their criteria asked the artifact-only reviewer to verify run-level or AI-role-provenance facts the bundle cannot show. Rule for relay plans: write criteria a reader can check by reading the produced diff, and scope any run-folder-completeness criterion explicitly to "judged from the run folder, not this diff."
- **Ground the relay before it builds; it will otherwise invent a plausible stack.** Giving the relay a plan whose "Requested work" embedded verified repo facts (existing `sus_*` schema, service/route paths, reuse targets) produced a correct brownfield-extension plan instead of a greenfield fantasy. A few scout agents over the real surfaces before writing the relay's input plan is worth it for any planning run on an unfamiliar area.
- **The relay builder writes the deliverable over the instruction file.** For a "produce a plan" task, the builder replaced the input plan's Requested-work/Acceptance-criteria sections with the output plan sections in the same file. Expect the committed file to be the deliverable, not the instructions; if you want both, use separate files or re-add the evidence after adoption.
- **A reviewer PASS is not a substitute for reading the diff.** The plan passed review but still had two operator-correctable flaws: a schema-number band error (`10NNN` vs the real next feature number 618) and em-dashes that violate `docs/anti-ai-style.md`. Neither was a false claim about existing code, but both needed a human pass. Read planning diffs even on PASS.

## Relay stale-checkout + headless posture (2026-07-10)

- **The primary checkout can be many commits behind `origin/main` and missing whole tool trees.** This session's primary checkout (`c:\Projects\Consulting_app`, a detached HEAD) was ~20 commits behind `origin/main` and did not contain `orchestration/coding_relay/`, `docs/reference/CODING_RELAY.md`, or the plan the task pointed at. A read-only Explore agent searching that checkout truthfully reported "no coding relay exists anywhere" - which was false for `origin/main`. Before concluding a tool, plan, or file "does not exist," check `git show origin/main:<path>` and `git log HEAD..origin/main`, not just the working tree. Run the relay from a fresh worktree branched off `origin/main` (`git worktree add -f ../wt origin/main -b <branch>`), never from a stale primary HEAD - otherwise the relay tool itself may be absent.
- **A task-referenced file can be an untracked draft, not repo truth.** The task pointed at `docs/adr/sustainability/001-brownfield-extension.md`; `git status` showed it as `??` (untracked) and `git show HEAD:<path>` failed - it was never committed, and it contradicted the merged, repo-grounded plan (0018). An IDE-opened or task-named path is a hint, not a source of truth; confirm it is tracked and reconciled before treating it as the master plan.
- **Headless relay run: pass `--plan NNNN` explicitly and use `--no-pr` to review before publishing.** Guided mode (no args) needs a TTY; without one it just prints the plan menu and exits. To run from a non-interactive tool, invoke `python -m orchestration.coding_relay --plan 0019 --max-iterations 3 --no-pr`. `--no-pr` runs the full BUILD/SCAN/TEST/REVIEW loop and, on PASS, leaves the verified diff **staged (uncommitted)** in the relay worktree instead of committing+pushing+opening a PR. That is the right posture when you want to read the diff, verify every claim against real code, and open the draft PR by hand (`git commit --no-verify` -> push via the `gh auth git-credential` helper -> `gh pr create --draft`). `--dry-run --probe-clis` first validates the plan and preflight (including CLI `--model` capability) without creating anything.

## Relay coordinator + orchestration-test placement (2026-07-10)

- **The relay hard-wires builder = `claude` CLI and reviewer = `codex` CLI; there is no role-swap flag.** `--claude-model` picks the builder's model, `--codex-model` the reviewer's. So any layer routing over the relay (the coordinator in `orchestration/relay_coordinator/`) can natively run "a Claude-family model builds / a Codex-family model reviews" (e.g. Fable builds / Sol reviews) but CANNOT run the reverse orientation (Sol builds / Fable reviews) without editing the relay, which is a recorded self-edit escalation. The honest design records the reverse orientation as `unsupported_by_current_worker` with a reason and does not launch or fake it - never pretend a capability the worker lacks. Adding codex-as-builder / claude-as-reviewer is a separate relay ticket gated on `--allow-relay-self-edit`.
- **Orchestration tests must live in `backend/tests/`, not next to the orchestration code, or CI never runs them.** CI's backend job runs `python -m ruff check app tests` and `python -m pytest tests -q` from `backend/`, so tests under `orchestration/**/tests/` are silently skipped by CI even though they pass locally. The relay's own tests already follow this (`backend/tests/test_orchestration_relay_*.py`); put coordinator tests at `backend/tests/test_relay_coordinator_*.py`. These test files self-insert repo root onto `sys.path` (`ROOT = Path(__file__).resolve().parents[N]; sys.path.insert(0, str(ROOT))`) so `import orchestration...` resolves regardless of pytest's cwd - when relocating a test up or down a directory level, fix the `parents[N]` offset (from `orchestration/x/tests/` repo-root is `parents[3]`; from `backend/tests/` it is `parents[2]`). "Green CI" only means something once the tests are somewhere CI actually collects them.

## The relay's test suites must fit the repo, or backend tickets cannot pass (2026-07-13)

- **The relay's default backend suite (`pytest tests -q`) never finishes in this repo, and that boxed in every backend ticket.** The full backend suite is ~5000 tests, many DB-bound. Run inside the relay's throwaway worktree it hits the 1800s cap and returns `exit 124` on every iteration. That leaves two dead ends: with tests on the relay MAX_ITERs on the timeout, and with `--no-tests` the reviewer honestly refuses to pass code it has no test evidence for. Both happened (T4, T5); both times the code was fine and a human had to run the targeted test and merge by hand. **Fix it as configuration, not as a patch to the relay**: `relay.config.json` (added by the portability work) replaces `backend-pytest` with a scoped `-k` run. Measured: `230 passed, 4861 deselected, 53s` versus a suite that never finished. CI still runs the complete suite, so scoping the relay's in-worktree gate trades away nothing. If a relay backend ticket keeps MAX_ITERing on a timeout, look at the suite command before you blame the builder.
- **A "junction failed" line in the relay's worktree setup means the frontend suites are silently NOT running.** `worktree.py` junctions `dep_links` (`repo-b/node_modules`, `backend/.venv`) into the relay worktree **from the launch-pad**, not from the primary checkout. A launch-pad worktree created with `git worktree add` has no `node_modules`, so the junction target is missing, the link fails, and `frontend-lint` / `frontend-typecheck` / `frontend-unit` all report SKIPPED. The run still looks healthy. Before a frontend ticket, junction it into the launch-pad: `cmd //c "mklink /J repo-b\node_modules <primary>\repo-b\node_modules"`, then confirm the run log says `deps repo-b/node_modules: linked` rather than `junction failed`. A SKIPPED suite is not a passing suite.
- **Do not put CI-only criteria in a relay plan.** A criterion like "the migration applies on the DB Schema Gate CI job" cannot be evidenced from an artifact bundle that is assembled before CI ever runs, so the reviewer returns `unknown` and the run burns to MAX_ITER with zero unmet criteria and no defect found (T6). Write criteria a reviewer can judge from the diff, and let CI own the CI gates.

## Relay escalations, telemetry, and proving a surface actually works (2026-07-14)

- **`orchestration_self_edit` is the one escalation you can legitimately grant, and the test is whether the plan targets the relay.** The relay refuses to modify its own code without the recorded `--allow-relay-self-edit`, so a builder cannot quietly weaken the harness that reviews it. When the telemetry ticket hit it, the override was correct: the plan's scope item 1 was explicitly "record the builder model in `orchestration/coding_relay/`", and the defect (`claude_model: "(cli default)"`) is *literally a hardcoded line in `__main__.py`*, unfixable from outside it. Grant it only when all of these hold: the plan names the relay as the target, the change is unavoidable from outside, and **no gating file moves** (`safety.py`, `loop.py`, `verdict.py`, `preflight.py` must show zero changes). Then run the relay's own 123 + coordinator's 80 tests as the regression gate - the reviewer will (rightly) refuse to accept them as green by inference.
- **A safety stop is not a rejection, and conflating them poisons routing evidence.** A `blocked` / `risk_escalation` verdict means the reviewer judged the work *unfit* - a signal about the builder model's quality. A `safety_stop` means the builder touched a *protected surface* - a signal about scope. The code can be perfectly good and still be stopped. Fold the two together in telemetry and you penalise whichever model happened to touch protected code, corrupting the very evidence you are collecting to decide model routing. Record and report them as separate columns.
- **Never record a model you cannot prove was used.** The relay wrote `claude_model: "(cli default)"` for 14 straight runs, so the builder was a black box and no Fable-vs-Sol evidence existed - and none could be reconstructed after the fact. When the CLI does not report a resolved model, record `null` + `cli_default_unreported`, never a guess. A fabricated attribution is worse than a missing one because it looks like data. Backfill can recover iterations, findings and test outcomes from old run folders; it cannot recover the model, and it must not pretend to.
- **A misaligned report is worse than no report.** Adding a column to a table's header without adding it to the row renderer shifts every value after it one place left, and the table then reads as authoritative while attributing one metric's number to another. The tell was a `safety_stop_rate` of `2.7` - impossible for a rate. Pin header/row width in a test, and assert that anything under a rate column is actually within `0..1`; that turns a silent corruption into a loud failure.
- **HTTP 307 proves a route exists, not that it works. An error state is not a fail-closed state.** After registering the sustainability workspace I `curl`ed the route, got `307` (redirect to login), and called it verified. It was not: the page rendered `Could not load authoritative sustainability state - Not Found`. A **404 from the API means the query never reached a working endpoint**; the fail-closed `snapshot_unavailable` state means the query *worked* and honestly found nothing. Those look similar on screen and are opposite diagnoses. Before calling an environment-scoped surface verified, load it authenticated and confirm it renders **real data**, or that its empty state is the *fail-closed* one and not an error. A page that returns *any* empty-looking state is not evidence the underlying query is correct.
- **Scope your `ruff --fix` to the files you changed.** `python -m ruff check orchestration --fix` silently rewrote four unrelated `orchestration/engine/*.py` files that were not in the plan - exactly the scope creep the relay's reviewer blocks builders for. Lint the paths you touched, not the tree.
