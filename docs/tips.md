...
- **Lakebase apply recipe (no PAT, no password):** the credential CLI is `databricks database generate-database-credential --json '{"instance_names":["novendor-telemetry"],"request_id":"<uuid>"}'` (NOT `--instance`); user = the Databricks email (`%40`-encode the `@`), db `databricks_postgres`, host `ep-royal-pond-…`. `node apply.js --dry-run` validates parse offline first.
