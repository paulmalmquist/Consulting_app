...
PR 5C: `snowflake_executor.py` is the ONE real action — `execute_auto_suspend(req, warehouse, auto_suspend_seconds, prior_auto_suspend_seconds, environment, now, client)` builds `ALTER WAREHOUSE <allowlisted> SET AUTO_SUSPEND = <int>` from typed fields, validates it (and the rollback SQL, generated first) with `validate_sql` (strict allow-only regex; rejects everything else), and runs it through the gate (env flag `ADE_OPS_REAL_EXEC_ENABLED` default off → non-prod only → approved → preflight → snowflake provider → allowlisted warehouse). The client is injected (CI mock; `_default_client` lazily imports `snowflake.connector` only behind the flag). No subprocess, no user-supplied SQL (signature has no `sql` param; test-enforced). Migration 616 tightens the guard to permit a real `nonprod` executed row only for `provider='snowflake'` + `action_kind='warehouse_auto_suspend'` + both SQL hashes present (verified). Route `POST /approvals/{id}/execute` with `execution_mode='nonprod'` + typed warehouse fields dispatches to it and persists via `record_real_execution`; receipt + UI show before→after + SQL hash as a "Live · non-prod" state. Prod and any other action stay impossible.

PR 6A: `watcher.py::evaluate(req, observation, now)` reads an executed change's receipt + observation window and returns a verdict (accepted / still_observing / degraded / rollback_recommended / insufficient_evidence) with evidence + null_reason. Pure evaluation — it reads only, recommends only, and a test asserts the module has no execution token (no `execute_auto_suspend`/`client.execute`/subprocess). Honesty rule: missing observation evidence → `insufficient_evidence`, never `accepted`; a worse outcome → `rollback_recommended` as an artifact (executes nothing). Route `POST /approvals/{id}/watch` (optional observation) + `GET /approvals/watch` (watcher state); both receipted via `governance.record_decision`. UI `WatcherPanel` distinguishes simulated / live non-prod / unavailable evidence. Incident state machine + auto-rollback are PR 6B / a later explicit decision.
