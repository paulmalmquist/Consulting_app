...
- [x] **C / Ticket 4 — Tasks UI domain grouping** — **DONE & VERIFIED 2026-05-19.** PR #75 merged (`8f30791d`). Display-only domain filter strip + per-card crumb; lanes preserved; backend additive (hierarchy cols + ref-table label joins, all optional). Deployed backend Railway `6bfc0d7a` + frontend Vercel `repo-jr5udqqyd`. Prod smoke: board API HTTP 200 with hierarchy fields, 251 tasks (all currently Ungrouped — assignment is write-path/Ticket 5). 16 backend tests pass, CI Frontend Typecheck SUCCESS. See dispatch `0002` Workstream C / Ticket 4.
- [x] **Ticket 5 — Hierarchy write-path** — **DONE & VERIFIED 2026-05-19.** PR #77 merged (`36f18714`). Drawer Domain→Initiative→Workstream selectors + fail-closed `validate_hierarchy` (unknown key → 400) + `GET /execution/hierarchy-options`. Deployed Railway `a27bf166` + Vercel `repo-ekakz26ko`. Prod smoke: live task `b4608987` assigned `Web Properties → FlowYorker.com → Content / SEO` (durable; board 1 grouped/251), invalid key → 400. 23 backend tests pass. See dispatch `0002` Ticket 5.
...
- [x] **H / Ticket 7 — Morning Brief Assistant read-only retrieval** — **DONE & VERIFIED in production 2026-05-21.** PR #83 merged (`5ad37b00`); 51 tests pass. New `POST /api/consulting/execution/brief-assistant/ask` (read-only, no writes, no tools). Production smoke (after the Ticket 7B fix unblocked the consulting router): 3 canonical questions return HTTP 200 with grounded answers, `tool_calls: []`, `refused: false`. See dispatch `0002` Ticket 7.
- [x] **Ticket 7 prod-deploy follow-up** — **DONE 2026-05-21.** Deploy proceeded but surfaced the Ticket 7B incident (consulting router hang); resolved together. Smoke complete.
- [x] **Ticket 7B — consulting router pool-exhaustion incident** — **FIXED + VERIFIED 2026-05-21.** Entire `/api/consulting/*` router hung in prod (board/morning-checklist/brief-assistant timed out 40–90s; sibling routers fine). Root cause: `run_auto_generation` held one pooled connection across all 8 auto-generation passes → pool (`max_size=10`) drained under concurrent board traffic → router-wide hang. Fix (PR #91, merge `700de249`, Railway deploy `978431c1`): per-pass `with get_cursor()` scoping in `run_auto_generation` (Stage 1) + pool `max_size` 10→20 / `timeout` 5→10 (Stage 2, defense-in-depth, verified safe vs Supabase `max_connections=60`). Regression guard `test_execution_auto_cursor_scoping.py`. Prod smoke: board returns 200 in 1.4s (was 90s hang); 15 concurrent board requests all 200, none hang. See dispatch `0002` Ticket 7B.

## Completed
- [x] **Consulting Revenue OS shell parity + data-driven workspace label** — **DONE & DEPLOYED 2026-05-22.** PR #97 merged (`2e972333`), Vercel deploy `repo-amhtmnid1`. New shared `ConsultingPageFrame` unifies the Pipeline/Contacts/Tasks shell chrome (Tasks + Contacts now use it; Pipeline keeps a bespoke 3-col shell by design — collapsible rail + sticky board — but imports the shared `winstonBrand`). Fixed 5 hardcoded "Hall Boys" strings in `OperatorShell.tsx` → data-driven from `environment.client_name` with generic "Operator" fallback (Hall Boys was stale demo copy, never env metadata). Frontend-only, no backend/schema/behavior change. CI all green. All 4 routes return 307 (auth redirect — page renders); true visual parity needs an authed browser check. Open follow-up if wanted: re-home `/operator/accounting` under `/consulting/*` for full Accounting shell parity (deliberately out of scope). tips #29/#30 added.
