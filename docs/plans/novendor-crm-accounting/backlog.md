...
- [x] **H / Ticket 7 — Morning Brief Assistant read-only retrieval** — **MERGED + verified (CI), PROD DEPLOY PENDING 2026-05-20.** PR #83 merged (`5ad37b00`); 51 tests pass; CI green on DB Schema Gate + Frontend Typecheck. New `POST /api/consulting/execution/brief-assistant/ask` (read-only, no writes, no tools, no LLM call in this layer). Backend deploy blocked by an active Railway incident (build queue paused for hobby/free plans 2026-05-20 ~12:16 UTC). Production endpoint returns 404 until Railway resumes; Tickets 1–6 endpoints unaffected. See dispatch `0002` Ticket 7.
- [ ] **Ticket 7 prod-deploy follow-up** — when Railway is healthy again, run `cd backend && railway up --service authentic-sparkle` from a clean worktree at `origin/main`, then production smoke `POST /bos/api/consulting/execution/brief-assistant/ask` with each of the 6 canonical questions; capture the request_id from the structured log and confirm grounded answers (no fabrication). Small, mechanical follow-up — not a re-design.
