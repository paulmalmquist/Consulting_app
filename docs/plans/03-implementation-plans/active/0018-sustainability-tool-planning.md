...
3. **T3 — Authoritative schema migration**: create the next-sequential `10NNN_sus_authoritative.sql` under `repo-b/db/schema/` with `sus_authoritative_snapshots`, `sus_authoritative_metric_value`, `sus_authoritative_evidence` — RLS on, `env_id` + `business_id`, comments, indexes justified.
