...
- **PR 8D** — Model Performance + Model Registry: artifact/run links + exportable metric rows (incl. the server `tel_anomaly_events` XLSX dataset batched here, with one controlled backend deploy). **Split per the don't-force-a-giant-PR rule: 8D-1 Model Performance · 8D-2 Model Registry · 8D-3 backend datasets + deploy.**
  ...
  - **8D-2 Model Registry — TODO (frontend).** `RegistryConsole.tsx` renders `mlflow_run_id` as raw text → wire `DatabricksRunLink`; `training_window`/`validation_method`/`operational_use` do **not exist** in `tel_model_runs` → render explicit null_reason ("not recorded in tel_model_runs"), not blank/"n/a"/"—"; add CSV + reuse the `model_runs` XLSX; drift cards can get a client CSV. Reuse 8A primitives.
  - **8D-3 backend datasets + deploy — TODO.** Add the `tel_anomaly_events` XLSX dataset (the 8C defer) to `telemetry_export.py` (allowlisted → an existing scoped service, read-only, bounded) + wire the Mission Control XLSX button; optional `drift` dataset; then one controlled `scripts/deploy_backend.sh` deploy (capture version/health, verify_lineage, the new dataset binary, rollback on fail).
