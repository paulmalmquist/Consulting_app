...
- **PR 8E** — RUL Calibration: wider layout + unit-level drill/export. **DONE (PR #416, main `7cadd04a`), frontend-only (one PR — no backend needed; the RUL data is a committed artifact, not a `tel_*` table).** Widened the container (1180→1320) + the trajectory chart (720×300→900×340); added a source-kind chip (FD001 scalars + bands = computed-artifact, the trajectory = replay fixture, not live serving); a "Unit-level rows + export" drawer (`MetricInspectorDrawer` + `SourceRowsTable`) over the per-cycle rows (unit, cycle, true_rul, point_pred, lo80/hi80, lo90/hi90, covered_80/covered_90 — coverage derived from the shown bands; flip/gate marked not-applicable here) + CSV (server XLSX honestly disabled — no `tel_*` dataset for the artifact). **Clarity:** `RulCalibration.tsx` is the CNN-LSTM FD001 page (PICP 0.778/0.903, RMSE 17.33 — all preserved); the **PICP 0.86 / 15-of-100** values are the Spin-3 `RulConformalCard` on the **Evidence** page (NOT touched). Frozen-evidence 8/8; 220 telemetry tests green. No deploy.
- **PR 8F** — Factory NCR + Flight Readiness: source/drill/export + layer-heat-map review. **DONE (split 8F-1 + 8F-2), frontend-only — no backend, no deploy.** Two distinct surfaces: `/telemetry/factory` = `FactoryNcrIntelligence` (RS-themed, API-backed NCR via `/api/telemetry/ncr`); `/telemetry/factory-ml` = `FactoryMlConsole` (C-themed, fixture-backed readiness + heatmap).
  - **8F-1 Factory NCR (PR #418, main `0e32f3b6`):** source-kind chip via `SOURCE_KIND_TAG` (`computed artifact` when provenance=databricks — real batch run, not live serving / `fixture` on local fallback); a thesis caption; a **severity filter** over the cluster's exemplars (data-supported; no invented status/time filters); per-exemplar **drill drawer** (`MetricInspectorDrawer`) exposing NCR id/severity/workcell/summary/cluster/cluster-status + the cluster's `DatabricksRunLink`, with detected_at/disposition/record-status fail-closed + explicit null_reason (corpus-grain mirror); **clusters CSV** + filter-aware **exemplars CSV**, disabled-with-reason when empty.
  - **8F-2 Flight Readiness (PR #419, main `4565abe1`):** **heat-map KEPT** (real argument — the SCN-005 golden-pair rhyme `00041`/`00088`; row-drillable) — added the missing fixture source-kind label + an "Export heatmap cells" CSV (long-form `heatmapExportRows`: run_id/pattern/scenario_id/is_anchor/window_w/z, real z not the ±2.5 color clamp) + a fuller "what a cell means" caption; readiness got an honest null_reason that per-ingredient **weight is not published** in the gold rollup (`weight_not_in_rollup`) — counts shown, no fabricated weights. Readiness gauge + vehicle drill + readiness CSV already existed (8A), unchanged.
  - Preserved every honesty footnote + frozen value (VEH-TR-003 0.58, golden pair, pr_auc 0.84, etc.); frozen-evidence 8/8; 224 telemetry tests green.
- **PR 8G** — Test Runs / Run Autopsy / Review flows, incl. Databricks run links where available. **DONE (8G-1 only; frontend-only — no backend, no deploy).** Route-first inventory: `/telemetry/runs` (`RunsExplorer`, **visible**, **live-rows** `tel_test_runs`); `RunAutopsy` (`/telemetry/data-engineering/autopsy`, **hidden-but-resolving**, data-engineering receipts — out of the demo-story scope, **not revived**); `ReplayForensicsDrawer` (embedded replay autopsy, computed-artifact/fixture).
  - **8G-1 Test Runs (PR #421, main `75de3a03`):** the `TestRun` grain `{id, run_key, dataset, unit_or_channel, spacecraft, row_count, ingest_at, status, created_at}` carries **no run links** (no mlflow/Databricks/artifact/evidence) — it's an ingest row, not a model run. Added a `live rows · tel_test_runs` source-kind chip; a per-run **drill** (`MetricInspectorDrawer`, desktop+mobile) exposing the present fields and **fail-closing** the absent ones (run-type/model-kind, MLflow/Databricks run, model/evidence artifact) with an explicit null_reason; the run_key is the **copyable id** — **no run URL fabricated**; **CSV** of the displayed runs, disabled-with-reason when empty. New `RunsExplorer.test.tsx`; 227 telemetry tests green.
  - **8G-2 Run Autopsy / Review — assessed, no PR needed.** `ReplayForensicsDrawer` (573 lines) already satisfies the bar: source-kind honesty (fixture/computed-artifact + "degenerate, not comparable" caveats), the fixture-vs-live-registry run-id mismatch **surfaced not reconciled**, MLflow run/experiment + Delta links (live-by-default, `NaRow` fail-closed), row-level confusion-matrix evidence with its in-sample caveat, and a full source→champion→run→fixture lineage chain. Forcing a marginal CSV into a delicate honesty surface was rejected; it already meets the review/autopsy requirements. No 8G-3 (no backend). **Next: 8H** (page rationalization / folding, hide-before-delete).
- **PR 8H** — Page rationalization / folding, hide-before-delete only. **8H-1 (inventory + plan) DONE.** Finding: **the telemetry app is already substantially rationalized** — hide-before-delete was applied in earlier phases and is **locked by `telemetryNav.test.ts`** (asserts the 6 sections + exact visible slugs, that `governance`/`how-it-works`/`evidence`/`copilot`/`data-engineering*` are NOT in nav, and that the hidden routes still resolve via `telemetryHref`). **No stale bridge links from visible pages to hidden pages exist** (every reference to a hidden slug is intra-hidden-page or a false positive — e.g. `TelemetryArchitectureMap` `s === "evidence"` is a palette status string, not a link; it renders inside the hidden `HowItWorks`). So there is **no low-risk nav/link/fold work left to implement** — the remaining items are owner decisions, not code.

  ### 8H rationalization inventory (route-first)
  | Candidate | Route | Nav status | Source files | Useful content / where folded | Route resolves? | Dedicated service only? | Risk | Recommended action |
  |---|---|---|---|---|---|---|---|---|
  | Chat Intelligence Copilot | `/telemetry/copilot` | hidden-but-resolving | `Copilot.tsx` | AI refusal/evidence receipts → already on per-page EvidenceContract + Model Performance | yes | no (shares `/api/telemetry/*`) | low | **hidden-but-resolving (no action)** |
  | Trust Center / governance | `/telemetry/governance` | hidden-but-resolving | `GovernanceDashboard.tsx` | governance facts → folded into Metric Lineage + source strips | yes | no | low | **hidden-but-resolving (no action)** |
  | How This Works | `/telemetry/how-it-works` | hidden-but-resolving | `HowItWorks.tsx` (+ `TelemetryArchitectureMap`) | how-it-works framing → Overview thesis + source strip | yes | no | low | **hidden-but-resolving (no action)** |
  | Resume Evidence | `/telemetry/evidence` | hidden-but-resolving | resume-evidence cards | resume-proof claims → per-page EvidenceContract cards | yes | no | low | **hidden-but-resolving (no action)** |
  | Source & Platform Map | `TelemetryArchitectureMap.tsx` (embedded in How-This-Works) | hidden (embedded) | `TelemetryArchitectureMap.tsx` | architecture map → lives inside How-This-Works | yes (via how-it-works) | no | low | **no action** |
  | Agent Control Tower | `/telemetry/control-tower` | **VISIBLE** | `ControlTower.tsx` | staged go/no-go workflow (real, route-resolving) | yes | no | med | **DECISION — currently visible + test-locked; an old owner note (#401) said "hide from nav". It is the sole Agent Operations item; hiding empties that section. Needs explicit keep-vs-hide sign-off before any nav change.** |
  | Data Engineering section | `/telemetry/data-engineering` | hidden-but-resolving | `data-engineering/DataEngineeringOverview.tsx` | DE landing → hidden; intra-section links are self-contained | yes | no | low | **hidden-but-resolving (no action); do NOT revive to nav** |
  | Data Map & Grain | `/telemetry/data-engineering/grain` | hidden-but-resolving | DE grain page | grain map → Metadata Explorer (visible) covers the live need | yes | no | low | **hidden-but-resolving (no action)** |
  | Relationships & Lineage | `/telemetry/data-engineering/relationships` | hidden-but-resolving | DE relationships page | lineage → **Metric Lineage** (visible) is the live fold target | yes | no | low | **hidden-but-resolving (no action)** |
  | Pipelines & Quality | `/telemetry/data-engineering/pipelines` | hidden-but-resolving | DE pipelines page | pipeline status → Mission Control PIPELINE panel (visible) | yes | no | low | **hidden-but-resolving (no action)** |
  | Agent Workbench | `/telemetry/data-engineering/workbench` | hidden-but-resolving | DE workbench page | — | yes | no | low | **hidden-but-resolving; quarantine candidate (future, with approval)** |
  | Run Autopsy | `/telemetry/data-engineering/autopsy` | hidden-but-resolving | `RunAutopsy.tsx` | receipts/audit → distinct from ReplayForensicsDrawer (the live autopsy) | yes | no | low | **hidden-but-resolving (no action)** |
  | Workflows / Sources (DE) | `/telemetry/data-engineering/{workflows,sources}` | hidden-but-resolving | DE pages | — | yes | no | low | **hidden-but-resolving; quarantine candidates (future, with approval)** |
  | Automated Data Engineering | `/lab/env/[envId]/automated-data-engineering` | **VISIBLE** (sidebar "Platform" cross-link) | `TelemetrySidebar.tsx:72`, ADE package | portable ADE control room (separate product) | yes | yes (its own ADE surface) | low | **DECISION — keep (it's a platform cross-link to a portable product, not a demo declutter target) unless owner wants it off the rail.** |
  | Monitoring / Spike Inspector | `/telemetry/{monitoring,spike-inspector}` | hidden-but-resolving | pages | consolidated into **System Health** (visible) | yes | no | low | **hidden-but-resolving (no action)** |

  **8H-1 inventory outcome + owner decisions applied. 8H COMPLETE.**
  - **8H-2 (PR #424, main `d7b1bcfb`):** owner decisions — **keep Agent Control Tower visible** (no change); **remove the ADE "Platform" sidebar cross-link**. Removed the cross-link (+ its section label) from `TelemetrySidebar` so the rail is telemetry-only; `/automated-data-engineering` still resolves directly. Sidebar test +1 (link gone, route not asserted).
  - **8H-3 (PR #425, main `05189823`):** owner approved deleting hidden DE pages "if they don't break anything." Only **`workflows`** qualified (zero inbound links; `WorkflowTemplates` imported nowhere but its own page+test) → **deleted** (3 files). **`workbench` + `sources` were NOT deleted** — they're wired into the `DataEngineeringOverview` landing (`ModeCard` + stat links + its test), so deletion would break that page; they stay **hidden-but-resolving** quarantine candidates pending the explicit DE-landing surgery. Proof of no breakage: typecheck clean, telemetry suite 225, nav test unaffected, mass-deletion gate passed (3 ≪ 100), the separate `/automated-data-engineering/workflows` product route untouched.
  - **8H-3 (fold):** ∅ — content already folded (resume/how-it-works → Overview + per-page `EvidenceContract`; lineage → Metric Lineage; monitoring/spike → System Health). No services/APIs removed (all shared). Frozen-evidence 8/8; no deploy. **Next: 8I** (screenshots / smoke / final acceptance).
- **PR 8I** — Screenshots / smoke / final acceptance pass. **DONE — verification + docs, no product scope, no deploy.** Full acceptance note: [`0012-telemetry-phase8-acceptance.md`](./0012-telemetry-phase8-acceptance.md). Prod verified at `e28f2c73`, verify_lineage 6/1/0; `model_runs` XLSX 200/6-rows, `anomaly_events` XLSX 200/header-only (consumer cold, honest), unknown dataset 404+null_reason — all valid binary through the proxy. 12 visible + 10 hidden-but-resolving routes confirmed present; deleted `workflows` gone. Local: typecheck clean, 225 telemetry + 10 nav/sidebar + 8 frozen-evidence + 11 export tests green. Drill/export receipt matrix + source-kind examples + preserved Spin values documented in the note. **PHASE 8 COMPLETE.**
...
### Bridge-deploy wiring (demo-visible) — `feat/stargate-bridge-deploy`
Made the Historical ML Evidence Bridge + the enriched Stargate console visible in the browser on novendor.ai.

- **Root cause:** `NEXT_PUBLIC_STARGATE_BRIDGE_URL` on Vercel `consulting-app` (prod) was set to an **empty string** for ~14 days. `bridgeBaseUrl()` treats empty as unset → `stream.configured=false` → the `!stream.configured` early return hid the **entire** Stargate console (the enriched StreamContextStrip / DLQ drawer / series export AND the ML Evidence Bridge).
- **Env fix:** set `NEXT_PUBLIC_STARGATE_BRIDGE_URL=https://authentic-sparkle-production-7f37.up.railway.app` (the Railway backend already serves `/stargate/*` in **capture mode** — verified `mode:capture` frames — and already returns `access-control-allow-origin: https://novendor.ai`, so the browser SSE works cross-origin, no new service deploy). Set via the Vercel REST API (`POST /v10/projects/{id}/env?upsert=true`) because `vercel env add` could not read stdin through the Windows shell. NEXT_PUBLIC is inlined at build, so the **merge-triggered rebuild** is what makes it live.
- **Resilience code change (`StargateConsole.tsx`):** extracted the ML bridge (card + `HistoricalMlEvidenceDrawer`) into a shared `mlEvidenceBridge` fragment rendered in **both** the configured and not-configured branches, so the static historical ML evidence is never hidden by a missing/empty live-bridge URL again. Test added (not-configured state still surfaces the bridge + opens the drawer).
- **Databricks links:** `NEXT_PUBLIC_DATABRICKS_WORKSPACE_URL` is unset → `resolveWorkspace()` returns the committed default (`dbc-2504bec5-b5ab`), so the deltaTableLinks render **live/clickable by default** in prod (override the env to point elsewhere; fail-closed copyable id only if the workspace resolves empty).

Tests: 260 telemetry + frozen-evidence 8/8; typecheck + lint clean. Deploy: env + frontend rebuild (no backend change).
