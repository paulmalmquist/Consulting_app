...
- **PR 8H** — Page rationalization / folding, hide-before-delete only. **8H-1 (inventory + plan) DONE.** Finding: **the telemetry app is already substantially rationalized** — hide-before-delete was applied in earlier phases and is **locked by `telemetryNav.test.ts`** (asserts the 6 sections + exact visible slugs, that `governance`/`how-it-works`/`evidence`/`copilot`/`data-engineering*` are NOT in nav, and that the hidden routes still resolve via `telemetryHref`). **No stale bridge links from visible pages to hidden pages exist** (every reference to a hidden slug is intra-hidden-page or a false positive — e.g. `TelemetryArchitectureMap` `s === "evidence"` is a palette status string, not a link; it renders inside the hidden `HowItWorks`). So there is **no low-risk nav/link/fold work left to implement** — the remaining items are owner decisions, not code.
  ...
  **8H-2/8H-3/8H-4: not needed as code right now.** No stale bridges to repoint (8H-2 ∅), content already folded into Evidence/Metric Lineage/Overview/EvidenceContract/System Health (8H-3 ∅), and the only deletion candidates (DE `workbench`/`workflows`/`sources`) stay **hidden-but-resolving** pending explicit quarantine approval (8H-4 = proposal only). The **two open decisions** (Agent Control Tower visibility; ADE "Platform" link) are surfaced for owner sign-off — no nav change made unilaterally. No services/APIs removed (all shared by Evidence/Metric Lineage/drill/replay/model pages). Frozen-evidence untouched; no deploy.
