...
import { seedCapitalCallsDemo } from "@/lib/server/reFinanceOperations";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await seedCapitalCallsDemo(pool, {
      envId: (body.env_id as string | undefined) || null,
      businessId: (body.business_id as string | undefined) || null,
    });
    return Response.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to seed capital call demo data";
