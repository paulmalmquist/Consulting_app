...
function CreateModelDialog({
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onOpenChange(false);
      window.location.assign(`${basePath}/models/${created.model_id}${newFundId ? "/fund-scenario" : ""}`);
    } catch (err) {
...
export default function ReModelsPage() {
  ...
  const handleCloneModel = async (modelId: string) => {
    ...
    try {
      ...
      const fundId = cloned.primary_fund_id ?? cloned.fund_id;
      window.location.assign(`${basePath}/models/${cloned.model_id}${fundId ? "/fund-scenario" : ""}`);
    } catch (err) {
                ...
                {models.map((m) => (
                  <tr key={m.model_id} className={`${reIndexTableRowClass} group`} data-testid={`model-row-${m.model_id}`}>
                    <td className="px-4 py-3 align-middle">
                      <a
                        href={`${basePath}/models/${m.model_id}${getModelFundId(m) ? "/fund-scenario" : ""}`}
