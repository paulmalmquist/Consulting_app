...
export default function LabEnvironmentShell({ envId, children }: Props) {
  ...
  const isDomainRoute = new RegExp(`^/lab/env/${envId}/(re|pds|credit|legal|medical|consulting|operator|opportunity-engine|ecc|demo|documents|definitions|resume|markets|trading|ncf|supply-chain|investment-engine|altered-mind|telemetry|healthcare-subscription|historyrhymes)(/|$)`).test(pathname);
