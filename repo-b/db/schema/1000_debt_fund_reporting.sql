...
COMMENT ON COLUMN re_fund_quarter_state.total_upb IS
  'Total Unpaid Principal Balance across all loans in fund (debt funds)';
COMMENT ON COLUMN re_fund_quarter_state.weighted_avg_coupon IS
  'UPB-weighted average coupon across loan book (debt funds)';
COMMENT ON COLUMN re_fund_quarter_state.watchlist_count IS
  'Count of active covenant alerts in fund (debt funds)';
COMMENT ON COLUMN re_fund_quarter_state.io_exposure_pct IS
  'Percentage of UPB in interest-only period (debt funds)';
