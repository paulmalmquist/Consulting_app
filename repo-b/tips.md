...
## Telemetry Spike Inspector — evaluate-and-recommend surfaces (June 2026)

- Model status as a closed set that includes a first-class `insufficient_evidence`. Don't overload `ok`/`error`. Stale or missing evidence must resolve to it (fail closed), never to a success state. Carry a separate `null_reason` so the UI can show *why* a judgement was withheld.
- Give each action four fixed facets — **what it does, what it does NOT do, required evidence, risk + gate**. Encoding these in a static `ACTION_CATALOG` keeps copy honest and stops the UI from implying execution it can't perform. Render the "what it will NOT do" list on the spike too, not just the action card.
- Keep **finding** (observation) separate from **recommendation** (proposed action). Any non-executed step should read as a *stage/recommend/review* affordance behind a human gate — never an "Execute" button. A rollback affordance on a read-only surface is a "recommend rollback review," not a rollback.
- Don't reuse operational verbs where they'd mislead. The telemetry detector verdicts are `nominal | review | no_go_review | not_available` precisely so the page never reads like a live go/no-go `GO`. The shared `verdictColor()` (GO/REVIEW/NO_GO) stays for the Control Tower; spike detectors get their own `detectorColor()`.
- New telemetry sub-pages need **only** a `TELEMETRY_NAV` entry in `telemetryNav.ts` plus a thin server `page.tsx` that awaits `params` and renders a `"use client"` component. The `TelemetryShell` wires the desktop rail, mobile drawer, and bottom bar from that one list.
- A three-column "feed / workspace / action-queue" layout is a literal Tailwind grid: `grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,300px)_minmax(0,1fr)_minmax(0,320px)]`. The shared `SplitGrid` only covers 2-up and even thirds. Keep the class string literal — the content scanner only sees literal strings, never props-composed class names.
- If a surface uses static/demo data, say so in the footer and label in-memory interactions (e.g. a staged-action queue) as "resets on refresh." Honesty about scope is cheaper than a user assuming the demo mutated production.
