...
on run argv
	...
	set flaggedState to item 4 of argv
	...
	tell application "Microsoft Outlook"
		...
		try
			set flagged status of targetMessage to (flaggedState is "1")
			return "ok"
		on error
			try
				set flag status of targetMessage to (flaggedState is "1")
				return "ok"
			on error errText
				error "Flag update failed: " & errText
			end try
		end try
	end tell
end run
